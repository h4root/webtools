...
| `DATA_DIR` | `./data`     | Каталог для истории (`store.json`), вложений и ключа. |
...
- `{ "type": "hello", "nick": "alice", "resume": "<токен прошлой сессии>" }` (`resume` опционален; нужен, чтобы после обрыва забрать свой ник обратно, пока сервер ещё держит мёртвое соединение)
