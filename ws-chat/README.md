...
- **Вложения**: файлы и картинки. Загрузка через `POST /upload` (тело - файл, заголовки `X-Filename`/`Content-Type`), файлы лежат в `DATA_DIR/uploads`, раздаются с `/uploads` (не-картинки - как `attachment` + `nosniff`, чтобы svg/html не исполнялись в origin). Картинки показываются инлайн, прочее - карточкой-ссылкой; лимит 25 МБ, до 10 вложений.
...
- Загрузка вложений вне WebSocket: `POST /upload` (тело - файл, заголовки `X-Filename`, `Content-Type`) → `{ id, name, size, mime }`; `id` затем кладётся в `attachments` сообщения.
